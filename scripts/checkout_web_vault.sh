...
FALLBACK_WEBVAULT_VERSION=v2025.12.1
...
if [ ! -d "${VAULT_FOLDER}" ]; then
    ...
    pushd "${VAULT_FOLDER}"
        ...
        git remote add vaultwarden https://github.com/dpinse/vw_web_builds.git
    popd
else
	...
	then
           echo "adding dpinse/vw_web_builds as remote repository"
           git remote add vaultwarden https://github.com/dpinse/vw_web_builds.git
        elif [ "${VAULTWARDEN_REMOTE}" != "https://github.com/dpinse/vw_web_builds.git" ]
	then
            echo "Warning: \`git remote get vaultwarden\` did not return the expected repository"
	    echo "expected: https://github.com/dpinse/vw_web_builds.git"
